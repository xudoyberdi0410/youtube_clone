...
import { ThumbsUp, MessageSquare, Share2, PlayCircle, PauseCircle } from 'lucide-react';
...
export const ShortsVideoCard: React.FC<ShortsVideoCardProps> = ({ short, videoRef, isPlaying, togglePlay, autoPlay }) => {
  return (
    <div className="w-full h-screen snap-center flex justify-center items-center relative bg-transparent">
      ...
      <div className="relative flex items-center justify-center" style={{ height: '100vh' }}>
        ...
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 w-[90%] max-w-[calc(100vh*9/16)] flex justify-between items-end pointer-events-none">
          <div className="space-y-2 bg-black/40 rounded-lg p-3 pointer-events-auto max-w-full truncate">
            ...
            <p className="text-sm opacity-80 truncate max-w-full break-words whitespace-pre-line">{short.author}</p>
          </div>
        </div>
        {/* Play/Pause Button */}
        <Button
      ...
      <div className="absolute left-1/2 top-1/2 -translate-y-1/2 translate-x-[calc(100vh*9/32+32px)] flex flex-col space-y-5 pointer-events-auto z-10">
        <Button variant="ghost" size="icon" className="flex flex-col items-center">
          ...
          <span className="text-xs mt-1">{short.likes}</span>
        </Button>
        <Button variant="ghost" size="icon" className="flex flex-col items-center">
